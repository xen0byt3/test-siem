    ...
    parts.forEach(part => {
      ...
      if(part.startsWith("ado")){
        ...
        // STOP MONITORING
          ...
          appendToTerminal("✖ Monitoring STOPPED", "error");
          commandHandled = true;
          return;
        }
...
function startAutoEscalation(){
  if(escalationInterval) return; // prevent duplicate intervals

  escalationInterval = setInterval(() => {
    if(isCriticalMode && monitoringState === "ACTIVE"){
      
      createTicket("CRITICAL", "Auto Escalation - Continuous Threat");

      showAlert("🚨 AUTO ESCALATION TRIGGERED");
