...
function exportTickets(){
  ...
  const now = new Date();
  const fileName = `adoVM_Tickets_${now.toISOString().slice(0,19)}.csv`;
...
function startAutoEscalation(){
  ...
  escalationInterval = setInterval(() => {
    ...
    createTicket("CRITICAL", getRandomSource());
...
function renderEscalatedTickets(){
  ...
  if(escalated.length === 0){
    escalationList.innerHTML = "<small>No escalated incidents</small>";
    document.getElementById("repEscalated").textContent = 0;
    return;
  }

  // render ALL (this is the fix)
  escalated.forEach(ticket => {

    let div = document.createElement("div");
    div.className = "ticket";

    let time = new Date(ticket.time).toLocaleTimeString();

    let color = ticket.severity === "CRITICAL" ? "#ef4444" : "#f59e0b";
    ...
    escalationList.appendChild(div);
  });

  // update counter
  document.getElementById("repEscalated").textContent = escalated.length;
}

function getRecommendation(source){

  const map = {
    "Brute Force Attack": "Enable account lockout & MFA",
    "Phishing Campaign": "User awareness training & email filtering",
    "C2 Beaconing": "Block outbound traffic & isolate host",
    "Ransomware Activity": "Isolate system & restore backups",
    "Data Exfiltration": "Monitor outbound traffic & DLP",
    "Insider Threat": "Audit user activity & restrict access",
    "Privilege Escalation": "Review permissions & patch systems",
    "Malware Infection": "Run antivirus & isolate endpoint",
    "APT Detection": "Perform deep forensic investigation",
    "Suspicious Login": "Verify user identity & enforce MFA"
  };

  return map[source] || "Investigate further";
}

function manualTicket(){

  let severity = prompt("Enter Severity (LOW / MEDIUM / HIGH / CRITICAL):");

  if(!severity) return;

  severity = severity.toUpperCase();

  let source = prompt("Enter Threat Source (or leave blank for random):");

  createTicket(severity, source || getRandomSource());
...
function submitTicket(){
  ...
  let severity = document.getElementById("ticketSeverity").value;
  let source = document.getElementById("ticketSource").value;
  let analyst = document.getElementById("ticketAnalyst").value;
  let notes = document.getElementById("ticketNotes").value;
  ...
  const ticket = {
    ...
    source: source,
    analyst: analyst,
    notes: notes || "Manual ticket created"
  };

  tickets.push(ticket);
  ticketCounter++;

  appendToTerminal(`🎟 Manual Ticket: ${ticket.id} [${severity}]`, "success");
...
function renderQueue(){
  ...
  const open = document.getElementById("queueOpen");
  const progress = document.getElementById("queueProgress");
  const escalated = document.getElementById("queueEscalated");
  const resolved = document.getElementById("queueResolved");
  ...
  tickets.forEach(ticket => {
    ...
    div.style.borderLeftColor = colorMap[ticket.severity] || "#ccc";
    ...
    div.innerHTML = `
      ...
      ${ticket.source}<br>
      <small>${ticket.severity} • ${time}</small><br>
      <small>👤 ${ticket.analyst || "Unassigned"}</small>

      <div class="ticket-actions">
        <button class="btn-start" onclick="startTicket('${ticket.id}')">Start</button>
        <button class="btn-resolve" onclick="resolveTicket('${ticket.id}')">Resolve</button>
        <button class="btn-escalate" onclick="forceEscalate('${ticket.id}')">Escalate</button>
      </div>
    `;

    if(ticket.status === "OPEN") open.appendChild(div);
    else if(ticket.status === "IN_PROGRESS") progress.appendChild(div);
    else if(ticket.status === "ESCALATED") escalated.appendChild(div);
    else if(ticket.status === "RESOLVED") resolved.appendChild(div);
